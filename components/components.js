...
function addSpeaker(input) {
    if (input && input.constructor !== Array) input = [input]
    // create speaker
    const speaker = document.createElement('img')
    speaker.src = 'https://img-bujdyehyko.now.sh/speaker.png'
    ...
    speaker.onmousedown = _ => {
        for (const source of input) {
            ...
            else if (source.constructor === HTMLAudioElement || source.constructor === OscillatorNode) {
                if (source.muted) {
                    source.muted = false
                } else {
                    source.muted = true
...
function winState(link) {
    ...
    const youwin = new Image(); youwin.src = 'https://img-bujdyehyko.now.sh/youwin.png'
    const taptocnt = new Image(); taptocnt.src = 'https://img-bujdyehyko.now.sh/taptocontinue.png'
    function Star(y) {
        this.y = y
